 ...
 * @date Sat Jun 17 2023 16:42 +0800
 * @version 0.00.001
 *
...
u32 u32_std_median_filtering(u32 u32_number)
{
    ...
    u32_p_buffer = u32_p_value;

    for (u32 i = 0; (u32_number - 1) > i; ++i)
    {
        for (u32 j = 0; (u32_number - i) > j; ++j)
        {
            if (*(u32_p_buffer + j) > *(u32_p_buffer + j + 1))
            {
                u32_temp = *(u32_p_buffer + j);
                *(u32_p_buffer + j) = *(u32_p_buffer + j + 1);
                *(u32_p_buffer + j + 1) = u32_temp;
            }
        }
    }

    return (*u32_p_value((u32_number - 1) / 2));
