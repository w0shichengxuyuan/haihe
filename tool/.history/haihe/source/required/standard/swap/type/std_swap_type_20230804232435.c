/**
 * @file std_swap_type.c
 * @brief std_swap_type
 * @author opencode (openc0de@hotmail.com)
 * @since 2023-06-10 22:22 +0800
 *
 * @authors opencode (openc0de@hotmail.com)
 * @date 2023-08-04 23:24 +0800
 * @version 0.00.001
 *
 * @copyright copyright ©2023 by opencode, all rights reserved.
 *
 * @details
 * -----------------------------------------------------------------------------
 *    version   |           date         |       by       |       comments
 * ------------ | ---------------------- | -------------- | --------------------
 *   0.00.001   | 2023-06-10 22:22 +0800 |    opencode    | 初版
 * -----------------------------------------------------------------------------
 */

/**
 * @file std_swap_type.c
 * @brief 标准类型转换文件
 * @author opencode (openc0de@hotmail.com)
 * @since 2023-06-10 22:22 +0800
 *
 * @authors opencode (openc0de@hotmail.com)
 * @date 2023-08-04 23:23 +0800
 * @version 0.00.001
 *
 * @copyright copyright ©2023 by opencode, all rights reserved.
 *
 * @details
 * -----------------------------------------------------------------------------
 *    version   |           date            |       by       |    comments
 * ------------ | ---------------------- | -------------- | --------------------
 *   0.00.001   | 2023-08-04 23:23 +0800 |    opencode    |
 * ------------ | --------------------------- | -------------- | ---------------
 *   0.00.001   | Sat Jun 10 2023 22:22 +0800 |    opencode    | 初版
 * -----------------------------------------------------------------------------
 */

#include "std_swap_type_private.h"
#include "std_swap_type_public.h"

/**
 * @brief u8数据转u16数据
 *
 * @param[in]  u8_p_in      - 输入数据
 * @param[in]  u8_in_len    - 输入长度
 * @param[out] u16_p_out    - 输出数据
 * @param[out] u8_p_out_len - 输出长度
 * @return SUCCESS - 成功
 *         FAILURE - 失败
 */
state_e e_std_swap_type_u8_to_u16(const u8  *u8_p_in,   u8  u8_in_len,
                                        u16 *u16_p_out, u8 *u8_p_out_len)
{
    if (NULL == u8_p_in)
        return (FAILURE);

    if (0 == u8_in_len)
        return (FAILURE);

    if (NULL == u16_p_out)
        return (FAILURE);

    if (NULL == u8_p_out_len)
        return (FAILURE);

    u8   u8_quotient  = 0,
         u8_remainder = 0,
        *u8_p_buffer  = NULL;
    u16 *u16_p_buffer = NULL;

    u8_p_buffer  = u8_p_in;
    u16_p_buffer = u16_p_out;
    u8_quotient  = u8_in_len >> 1;
    u8_remainder = u8_in_len  & 1;

    for (u8 i = 0; u8_quotient > i; ++i)
    {
        *u16_p_buffer  = 0;

        *u16_p_buffer  = *u8_p_buffer++ << 8;
        *u16_p_buffer |= *u8_p_buffer++     ;

        ++u16_p_buffer;
    }

    *u8_p_out_len = u8_quotient;
    if (0 == u8_remainder)
        return (SUCCESS);

    *u8_p_out_len += 1;
    for (u8 i = 0; u8_remainder > i; ++i)
    {
        *u16_p_buffer = 0;

        *u16_p_buffer = *u8_p_buffer++ << ((1 - i) << 3);
    }

    return (SUCCESS);
} /* e_std_swap_type_u8_to_u16 */

/**
 * @brief u8数据转u32数据
 *
 * @param[in]  u8_p_in      - 输入数据
 * @param[in]  u8_in_len    - 输入长度
 * @param[out] u32_p_out    - 输出数据
 * @param[out] u8_p_out_len - 输出长度
 * @return SUCCESS - 成功
 *         FAILURE - 失败
 */
state_e e_std_swap_type_u8_to_u32(const u8  *u8_p_in,   u8  u8_in_len,
                                        u32 *u32_p_out, u8 *u8_p_out_len)
{
    if (NULL == u8_p_in)
        return (FAILURE);

    if (0 == u8_in_len)
        return (FAILURE);

    if (NULL == u32_p_out)
        return (FAILURE);

    if (NULL == u8_p_out_len)
        return (FAILURE);

    u8   u8_quotient  = 0,
         u8_remainder = 0,
        *u8_p_buffer  = NULL;
    u32 *u32_p_buffer = NULL;

    u8_p_buffer  = u8_p_in;
    u32_p_buffer = u32_p_out;
    u8_quotient  = u8_in_len >> 2;
    u8_remainder = u8_in_len  & 3;

    for (u8 i = 0; u8_quotient > i; ++i)
    {
        *u32_p_buffer  = 0;

        *u32_p_buffer  = *u8_p_buffer++ << 24;
        *u32_p_buffer |= *u8_p_buffer++ << 16;
        *u32_p_buffer |= *u8_p_buffer++ <<  8;
        *u32_p_buffer |= *u8_p_buffer++      ;

        ++u32_p_buffer;
    }

    *u8_p_out_len = u8_quotient;
    if (0 == u8_remainder)
        return (SUCCESS);

    *u8_p_out_len += 1;
    for (u8 i = 0; u8_remainder > i; ++i)
    {
        *u32_p_buffer = 0;

        *u32_p_buffer = *u8_p_buffer++ << ((3 - i) << 3);
    }

    return (SUCCESS);
} /* e_std_swap_type_u8_to_u32 */

/**
 * @brief u8数据转u64数据
 *
 * @param[in]  u8_p_in      - 输入数据
 * @param[in]  u8_in_len    - 输入长度
 * @param[out] u64_p_out    - 输出数据
 * @param[out] u8_p_out_len - 输出长度
 * @return SUCCESS - 成功
 *         FAILURE - 失败
 */
state_e e_std_swap_type_u8_to_u64(const u8  *u8_p_in,   u8  u8_in_len,
                                        u64 *u64_p_out, u8 *u8_p_out_len)
{
    if (NULL == u8_p_in)
        return (FAILURE);

    if (0 == u8_in_len)
        return (FAILURE);

    if (NULL == u64_p_out)
        return (FAILURE);

    if (NULL == u8_p_out_len)
        return (FAILURE);

    u8   u8_quotient  = 0,
         u8_remainder = 0,
        *u8_p_buffer  = NULL;
    u64 *u64_p_buffer = NULL;

    u8_p_buffer  = u8_p_in;
    u64_p_buffer = u64_p_out;
    u8_quotient  = u8_in_len >> 3;
    u8_remainder = u8_in_len  & 7;

    for (u8 i = 0; u8_quotient > i; ++i)
    {
        *u64_p_buffer  = 0;

        *u64_p_buffer  = *u8_p_buffer++ << 56;
        *u64_p_buffer |= *u8_p_buffer++ << 48;
        *u64_p_buffer |= *u8_p_buffer++ << 40;
        *u64_p_buffer |= *u8_p_buffer++ << 32;
        *u64_p_buffer |= *u8_p_buffer++ << 24;
        *u64_p_buffer |= *u8_p_buffer++ << 16;
        *u64_p_buffer |= *u8_p_buffer++ <<  8;
        *u64_p_buffer |= *u8_p_buffer++      ;

        ++u64_p_buffer;
    }

    *u8_p_out_len = u8_quotient;
    if (0 == u8_remainder)
        return (SUCCESS);

    *u8_p_out_len += 1;
    for (u8 i = 0; u8_remainder > i; ++i)
    {
        *u64_p_buffer = 0;

        *u64_p_buffer = *u8_p_buffer++ << ((7 - i) << 3);
    }

    return (SUCCESS);
} /* e_std_swap_type_u8_to_u64 */

/**
 * @brief u16数据转u8数据
 *
 * @param[in]  u16_p_in     - 输入数据
 * @param[in]  u8_in_len    - 输入长度
 * @param[out] u8_p_out     - 输出数据
 * @param[out] u8_p_out_len - 输出长度
 * @return SUCCESS - 成功
 *         FAILURE - 失败
 */
state_e e_std_swap_type_u16_to_u8(const u16 *u16_p_in, u8  u8_in_len,
                                        u8  *u8_p_out, u8 *u8_p_out_len)
{
    if (NULL == u16_p_in)
        return (FAILURE);

    if (0 == u8_in_len)
        return (FAILURE);

    if (255 < (u8_in_len << 1))
        return (FAILURE);

    if (NULL == u8_p_out)
        return (FAILURE);

    if (NULL == u8_p_out_len)
        return (FAILURE);

    u8   u8_number    = 0,
        *u8_p_buffer  = NULL;
    u16 *u16_p_buffer = NULL;

    u16_p_buffer = u16_p_in;
    u8_p_buffer  = u8_p_out;
    u8_number    = u8_in_len << 1;

    for (u8 i = 0; u8_number > i; ++i)
    {
        *u8_p_buffer++ = *u16_p_buffer >> 8;
        *u8_p_buffer++ = *u16_p_buffer++;
    }

    return (SUCCESS);
} /* e_std_swap_type_u16_to_u8 */

/**
 * @brief u16数据转u32数据
 *
 * @param[in]  u16_p_in     - 输入数据
 * @param[in]  u8_in_len    - 输入长度
 * @param[out] u32_p_out    - 输出数据
 * @param[out] u8_p_out_len - 输出长度
 * @return SUCCESS - 成功
 *         FAILURE - 失败
 */
state_e e_std_swap_type_u16_to_u32(const u16 *u16_p_in,  u8  u8_in_len,
                                         u32 *u32_p_out, u8 *u8_p_out_len)
{
    if (NULL == u16_p_in)
        return (FAILURE);

    if (0 == u8_in_len)
        return (FAILURE);

    if (NULL == u32_p_out)
        return (FAILURE);

    if (NULL == u8_p_out_len)
        return (FAILURE);

    u8   u8_quotient  = 0,
         u8_remainder = 0;
    u16 *u16_p_buffer = NULL;
    u32 *u32_p_buffer = NULL;

    u16_p_buffer = u16_p_in;
    u32_p_buffer = u32_p_out;
    u8_quotient  = u8_in_len >> 1;
    u8_remainder = u8_in_len  & 1;

    for (u8 i = 0; u8_quotient > i; ++i)
    {
        *u32_p_buffer  = 0;

        *u32_p_buffer  = *u16_p_buffer++ << 16;
        *u32_p_buffer |= *u16_p_buffer++      ;

        ++u32_p_buffer;
    }

    *u8_p_out_len = u8_quotient;
    if (0 == u8_remainder)
        return (SUCCESS);

    *u8_p_out_len += 1;
    for (u8 i = 0; u8_remainder > i; ++i)
    {
        *u32_p_buffer = 0;

        *u32_p_buffer = *u16_p_buffer++ << ((1 - i) << 4);
    }

    return (SUCCESS);
} /* e_std_swap_type_u16_to_u32 */

/**
 * @brief u16数据转u64数据
 *
 * @param[in]  u16_p_in     - 输入数据
 * @param[in]  u8_in_len    - 输入长度
 * @param[out] u64_p_out    - 输出数据
 * @param[out] u8_p_out_len - 输出长度
 * @return SUCCESS - 成功
 *         FAILURE - 失败
 */
state_e e_std_swap_type_u16_to_u64(const u16 *u16_p_in,  u8  u8_in_len,
                                         u64 *u64_p_out, u8 *u8_p_out_len)
{
    if (NULL == u16_p_in)
        return (FAILURE);

    if (0 == u8_in_len)
        return (FAILURE);

    if (NULL == u64_p_out)
        return (FAILURE);

    if (NULL == u8_p_out_len)
        return (FAILURE);

    u8   u8_quotient  = 0,
         u8_remainder = 0;
    u16 *u16_p_buffer = NULL;
    u64 *u64_p_buffer = NULL;

    u16_p_buffer = u16_p_in;
    u64_p_buffer = u64_p_out;
    u8_quotient  = u8_in_len >> 2;
    u8_remainder = u8_in_len  & 3;

    for (u8 i = 0; u8_quotient > i; ++i)
    {
        *u64_p_buffer  = 0;

        *u64_p_buffer  = *u16_p_buffer++ << 48;
        *u64_p_buffer |= *u16_p_buffer++ << 32;
        *u64_p_buffer |= *u16_p_buffer++ << 16;
        *u64_p_buffer |= *u16_p_buffer++      ;

        ++u64_p_buffer;
    }

    *u8_p_out_len = u8_quotient;
    if (0 == u8_remainder)
        return (SUCCESS);

    *u8_p_out_len += 1;
    for (u8 i = 0; u8_remainder > i; ++i)
    {
        *u64_p_buffer = 0;

        *u64_p_buffer = *u16_p_buffer++ << ((3 - i) << 4);
    }

    return (SUCCESS);
} /* e_std_swap_type_u16_to_u64 */

/**
 * @brief u32数据转u8数据
 *
 * @param[in]  u32_p_in     - 输入数据
 * @param[in]  u8_in_len    - 输入长度
 * @param[out] u8_p_out     - 输出数据
 * @param[out] u8_p_out_len - 输出长度
 * @return SUCCESS - 成功
 *         FAILURE - 失败
 */
state_e e_std_swap_type_u32_to_u8(const u32 *u32_p_in, u8  u8_in_len,
                                        u8  *u8_p_out, u8 *u8_p_out_len)
{
    if (NULL == u32_p_in)
        return (FAILURE);

    if (0 == u8_in_len)
        return (FAILURE);

    if (255 < (u8_in_len << 2))
        return (FAILURE);

    if (NULL == u8_p_out)
        return (FAILURE);

    if (NULL == u8_p_out_len)
        return (FAILURE);

    u8   u8_number    = 0,
        *u8_p_buffer  = 0;
    u16 *u32_p_buffer = NULL;

    u32_p_buffer = u32_p_in;
    u8_p_buffer  = u8_p_out;
    u8_number    = u8_in_len << 2;

    for (u8 i = 0; u8_number > i; ++i)
    {
        *u8_p_buffer++ = *u32_p_buffer >> 24;
        *u8_p_buffer++ = *u32_p_buffer >> 16;
        *u8_p_buffer++ = *u32_p_buffer >>  8;
        *u8_p_buffer++ = *u32_p_buffer++;
    }

    return (SUCCESS);
} /* e_std_swap_type_u32_to_u8 */

/**
 * @brief u32数据转u16数据
 *
 * @param[in]  u32_p_in     - 输入数据
 * @param[in]  u8_in_len    - 输入长度
 * @param[out] u16_p_out    - 输出数据
 * @param[out] u8_p_out_len - 输出长度
 * @return SUCCESS - 成功
 *         FAILURE - 失败
 */
state_e e_std_swap_type_u32_to_u16(const u32 *u32_p_in,  u8  u8_in_len,
                                         u16 *u16_p_out, u8 *u8_p_out_len)
{
    if (NULL == u32_p_in)
        return (FAILURE);

    if (0 == u8_in_len)
        return (FAILURE);

    if (255 < (u8_in_len << 1))
        return (FAILURE);

    if (NULL == u16_p_out)
        return (FAILURE);

    if (NULL == u8_p_out_len)
        return (FAILURE);

    u8   u8_number    = 0;
    u16 *u16_p_buffer = NULL;
    u32 *u32_p_buffer = NULL;

    u16_p_buffer = u32_p_in;
    u32_p_buffer = u16_p_out;
    u8_number    = u8_in_len << 1;

    for (u8 i = 0; u8_number > i; ++i)
    {
        *u16_p_buffer++ = *u32_p_buffer >> 16;
        *u16_p_buffer++ = *u32_p_buffer++;
    }

    return (SUCCESS);
} /* e_std_swap_type_u32_to_u16 */

/**
 * @brief u32数据转u64数据
 *
 * @param[in]  u32_p_in     - 输入数据
 * @param[in]  u8_in_len    - 输入长度
 * @param[out] u64_p_out    - 输出数据
 * @param[out] u8_p_out_len - 输出长度
 * @return SUCCESS - 成功
 *         FAILURE - 失败
 */
state_e e_std_swap_type_u32_to_u64(const u32 *u32_p_in,  u8  u8_in_len,
                                         u64 *u64_p_out, u8 *u8_p_out_len)
{
    if (NULL == u32_p_in)
        return (FAILURE);

    if (0 == u8_in_len)
        return (FAILURE);

    if (NULL == u64_p_out)
        return (FAILURE);

    if (NULL == u8_p_out_len)
        return (FAILURE);

    u8   u8_quotient  = 0,
         u8_remainder = 0;
    u32 *u32_p_buffer = NULL;
    u64 *u64_p_buffer = NULL;

    u32_p_buffer = u32_p_in;
    u64_p_buffer = u64_p_out;
    u8_quotient  = u8_in_len >> 1;
    u8_remainder = u8_in_len  & 1;

    for (u8 i = 0; u8_quotient > i; ++i)
    {
        *u64_p_buffer  = 0;

        *u64_p_buffer  = *u32_p_buffer++ << 32;
        *u64_p_buffer |= *u32_p_buffer++      ;

        ++u64_p_buffer;
    }

    *u8_p_out_len = u8_quotient;
    if (0 == u8_remainder)
        return (SUCCESS);

    *u8_p_out_len += 1;
    for (u8 i = 0; u8_remainder > i; ++i)
    {
        *u64_p_buffer = 0;

        *u64_p_buffer = *u32_p_buffer++ << ((1 - i) << 5);
    }

    return (SUCCESS);
} /* e_std_swap_type_u32_to_u64 */

/**
 * @brief u64数据转u8数据
 *
 * @param[in]  u64_p_in     - 输入数据
 * @param[in]  u8_in_len    - 输入长度
 * @param[out] u8_p_out     - 输出数据
 * @param[out] u8_p_out_len - 输出长度
 * @return SUCCESS - 成功
 *         FAILURE - 失败
 */
state_e e_std_swap_type_u64_to_u8(const u64 *u64_p_in, u8  u8_in_len,
                                        u8  *u8_p_out, u8 *u8_p_out_len)
{
    if (NULL == u64_p_in)
        return (FAILURE);

    if (0 == u8_in_len)
        return (FAILURE);

    if (255 < (u8_in_len << 3))
        return (FAILURE);

    if (NULL == u8_p_out)
        return (FAILURE);

    if (NULL == u8_p_out_len)
        return (FAILURE);

    u8   u8_number    = 0,
        *u8_p_buffer  = NULL;
    u64 *u64_p_buffer = NULL;

    u64_p_buffer = u64_p_in;
    u8_p_buffer  = u8_p_out;
    u8_number    = u8_in_len << 3;

    for (u8 i = 0; u8_number > i; ++i)
    {
        *u8_p_buffer++ = *u64_p_buffer >> 56;
        *u8_p_buffer++ = *u64_p_buffer >> 48;
        *u8_p_buffer++ = *u64_p_buffer >> 32;
        *u8_p_buffer++ = *u64_p_buffer >> 24;
        *u8_p_buffer++ = *u64_p_buffer >> 16;
        *u8_p_buffer++ = *u64_p_buffer >>  8;
        *u8_p_buffer++ = *u64_p_buffer++;
    }

    return (SUCCESS);
} /* e_std_swap_type_u64_to_u8 */

/**
 * @brief u64数据转u16数据
 *
 * @param[in]  u64_p_in     - 输入数据
 * @param[in]  u8_in_len    - 输入长度
 * @param[out] u16_p_out    - 输出数据
 * @param[out] u8_p_out_len - 输出长度
 * @return SUCCESS - 成功
 *         FAILURE - 失败
 */
state_e e_std_swap_type_u64_to_u16(const u64 *u64_p_in,  u8  u8_in_len,
                                         u16 *u16_p_out, u8 *u8_p_out_len)
{
    if (NULL == u64_p_in)
        return (FAILURE);

    if (0 == u8_in_len)
        return (FAILURE);

    if (255 < (u8_in_len << 2))
        return (FAILURE);

    if (NULL == u16_p_out)
        return (FAILURE);

    if (NULL == u8_p_out_len)
        return (FAILURE);

    u8   u8_number    = 0;
    u16 *u16_p_buffer = NULL;
    u64 *u64_p_buffer = NULL;

    u64_p_buffer = u64_p_in;
    u16_p_buffer = u16_p_out;
    u8_number    = u8_in_len << 2;

    for (u8 i = 0; u8_number > i; ++i)
    {
        *u16_p_buffer++ = *u64_p_buffer >> 48;
        *u16_p_buffer++ = *u64_p_buffer >> 32;
        *u16_p_buffer++ = *u64_p_buffer >> 16;
        *u16_p_buffer++ = *u64_p_buffer++;
    }

    return (SUCCESS);
} /* e_std_swap_type_u64_to_u16 */

/**
 * @brief u64数据转u32数据
 *
 * @param[in]  u64_p_in     - 输入数据
 * @param[in]  u8_in_len    - 输入长度
 * @param[out] u32_p_out    - 输出数据
 * @param[out] u8_p_out_len - 输出长度
 * @return SUCCESS - 成功
 *         FAILURE - 失败
 */
state_e e_std_swap_type_u64_to_u32(const u64 *u64_p_in,  u8  u8_in_len,
                                         u32 *u32_p_out, u8 *u8_p_out_len)
{
    if (NULL == u64_p_in)
        return (FAILURE);

    if (0 == u8_in_len)
        return (FAILURE);

    if (255 < (u8_in_len << 1))
        return (FAILURE);

    if (NULL == u32_p_out)
        return (FAILURE);

    if (NULL == u8_p_out_len)
        return (FAILURE);

    u8   u8_number    = 0;
    u32 *u32_p_buffer = NULL;
    u64 *u64_p_buffer = NULL;

    u64_p_buffer = u64_p_in;
    u32_p_buffer = u32_p_out;
    u8_number    = u8_in_len << 1;

    for (u8 i = 0; u8_number > i; ++i)
    {
        *u32_p_buffer++ = *u64_p_buffer >> 32;
        *u32_p_buffer++ = *u64_p_buffer++;
    }

    return (SUCCESS);
} /* e_std_swap_type_u64_to_u32 */
